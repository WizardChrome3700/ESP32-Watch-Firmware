...
class SH1106 {
	 ...
	 */
	SH1106(uint8_t sck, uint8_t din, uint8_t cs, uint8_t dc, uint8_t res) {
		this->sck = sck;
		this->din = din;
		this->cs = cs;
		this->dc = dc;
		...
		displayBuffer = SPI.getTxBuffer();
	}

	~SH1106() {
		if (this->displayBuffer != nullptr) {
			free(this->displayBuffer); // Safely returns the 1056 bytes back to the OS heap!
	...
	void updateDisplay() {
		...
		sh1106_command(0x02);            // Lower column address = 2
		sh1106_command(0x10);            // Higher column address = 0
		SPI.transfer(displayBuffer, 128*8);
	}
